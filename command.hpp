...
void Command::Move(const std::string &tank_id, const Direction &dir, const double &dis) {
  // Construct JSON
  rapidjson::Document d;
  rapidjson::Document::AllocatorType &a = d.GetAllocator();
  d.SetObject();
  d.AddMember("tank_id", rapidjson::StringRef(tank_id.c_str()), a);
  d.AddMember("comm_type", "MOVE", a);
  switch(dir) {
    case FWD:
      d.AddMember("direction", "FWD", a);
      break;
    case REV:
      d.AddMember("direction", "REV", a);
      break;
  }
  d.AddMember("distance", dis, a);
  d.AddMember("client_token", rapidjson::StringRef(token.c_str()), a);
...
void Command::Rotate(const std::string &tank_id, const Rotation &rot, const double &rad) {
  // Construct JSON
  rapidjson::Document d;
  rapidjson::Document::AllocatorType &a = d.GetAllocator();
  d.SetObject();
  d.AddMember("tank_id", rapidjson::StringRef(tank_id.c_str()), a);
  d.AddMember("comm_type", "ROTATE", a);
  switch(rot) {
    case CW:
